...
#   ungroup()
  ...
  stay_time_tele <- cell_captures_tele |>
    ...
    dplyr::summarise(t_stay = max(t) - min(t),
                     lscape_index = lscape_index[1]) |>
    # dplyr::add_row(lscape_index = cam.samps[cam.samps %notin% cam_captures_tele$lscape_index]) |>
  ...
  stay_time_tele <- as.matrix(dplyr::left_join(data.frame(lscape_index = cam.samps),
                                               stay_time_tele,
                                               by = "lscape_index") |>
