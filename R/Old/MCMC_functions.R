...
fit.model.mcmc.PR <- function(n.iter,
  ...
  d <- exp(gamma[1])
  tot.u <- d * sample_frame / (cam.A * t.steps)
  ...
  for (i in 1:n.iter) {
    ...
    d <- exp(gamma[i + 1])
    tot.u[i + 1] <- d * sample_frame / (cam.A * t.steps)
...
# MCMC for PR no covariates
########################################
fit.model.mcmc.PR.habitat <- function(n.iter,
                                      gamma.start,
                                      gamma.prior.var,
                                      gamma.tune,
                                      cam.counts,
  ...
  for (i in 1:n.iter) {
    ...
    for (gg in 1:sum(covariates.index == 1)) {
      ...
      if (length(cam.counts.h) == 0) {
        cam.counts.h <- 0
      }
      
      if (all(d.star > 0)) {
        mh1 <- sum(dpois(cam.counts.h, d.star[gg], log = TRUE), na.rm = TRUE) +
          sum(dnorm(gamma.star[gg], 0, gamma.prior.var^0.5, log = TRUE))
        mh2 <- sum(dpois(cam.counts.h, d[gg], log = TRUE), na.rm = TRUE) +
          sum(dnorm(gamma[i, gg], 0, gamma.prior.var^0.5, log = TRUE))
        mh <- exp(mh1 - mh2)
    ...
    if (i %% tune.check == 0) {
      ...
      # delta_n <- min(0.01,batch_n^-1)
      accept.gamma.check <- colMeans(accept[(i - tune.check + 1):i, 1:sum(covariates.index == 1)], na.rm = T)
                                #mean(accept[(i - tune.check + 1):i], na.rm = T)
      gamma.tune[accept.gamma.check > 0.44] <- gamma.tune[accept.gamma.check > 0.44] + delta_n
      gamma.tune[accept.gamma.check <= 0.44] <- gamma.tune[accept.gamma.check <= 0.44] - delta_n
    }
  }
