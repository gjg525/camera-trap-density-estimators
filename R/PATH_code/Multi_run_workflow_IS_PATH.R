...
for (run in 1:study_design$num_runs) {
  ...
  if (run %in% seq(101, 901, by = 100)) {
    dat_name <- paste0("save_animal_data_", ceiling(run / 100))
    load(file = paste0("Sim_results_PATH/", dat_name, ".RData"))
  ...
  habitat_summary <- lscape_defs %>%
    ...
    replace(is.na(.), 0) %>% 
    dplyr::select(Speed, n_lscape, prop_cams)
...
save(results_all_fast_cam, file = "Sim_results_PATH/results_all_fast_cam.RData")
