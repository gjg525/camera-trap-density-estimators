...
########################################
# MCMC for EEDE w/ covariates 
########################################
fit.model.mcmc.EEDE.cov <- function(n.iter,
                                    gamma.start,
                                    kappa.start,
                                    gamma.prior.var,
                                    kappa.prior.var,
  ...
  kappa[1,] <- kappa.start
  colnames(kappa) <- paste("kappa",names(spatial.covariates)[which(covariates.index==1)])
  colnames(tot.u) <- "Total estimate"
  colnames(accept) <- c(paste("accept.rate.gamma",names(spatial.covariates)[2]),
                        paste("accept.rate.kappa",names(spatial.covariates)[which(covariates.index==1)]))
...
fit.model.mcmc.REST <- function(n.iter,
  ...
  for(i in 1:n.iter){
    ...
    eta.star <- u.star*t.steps*cam.A/phi
...
fit.model.mcmc.REST.cov <- function(n.iter,
                                gamma.start,kappa.start,
                                gamma.prior.var,kappa.prior.var,
                                gamma.tune,kappa.tune,
                                num.encounters.dat,t.staying.dat,spatial.covariates,covariates.index,
                                cam.A,t.steps,censor){
  ...
  gamma[1,] <- gamma.start
  colnames(gamma) <- paste("gamma",names(spatial.covariates)[which(covariates.index==1)])
  kappa[1,] <- kappa.start
  colnames(kappa) <- paste("kappa",names(spatial.covariates)[which(covariates.index==1)])
  colnames(tot.u) <- "Total estimate"
  colnames(accept) <- c(paste("accept.rate.gamma",names(spatial.covariates)[which(covariates.index==1)]),
                        paste("accept.rate.kappa",names(spatial.covariates)[which(covariates.index==1)]))
...
fit.model.mcmc.MCT.cov <- function(n.iter,
  ...
  colnames(gamma) <- paste("gamma",names(spatial.covariates)[which(covariates.index==1)])
  colnames(tot.u) <- "Total estimate"
  colnames(accept) <- c(paste("accept.rate.gamma",
                              names(spatial.covariates)[which(covariates.index==1)]))
  ...
  for(i in 1:n.iter){
    ...
    for(gg in 1:sum(covariates.index==1)){
      ...
      # repeat estimated parms for fitting
      d.star.cams <- d.star[cam.samps]
      d.all.cams <- d[cam.samps]
...
fit.model.mcmc.STE <- function(n.iter,gamma.start,gamma.prior.var,gamma.tune,
                                    STE.dat,spatial.covariates,covariates.index,
                                    cam.A,cam.samps.in,censor){
